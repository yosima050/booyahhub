...
class StatusPendaftaranScreen extends StatefulWidget {
  ...
  @override
  State<StatusPendaftaranScreen> createState() => _StatusPendaftaranScreenState();
}
...
class _StatusPendaftaranScreenState extends State<StatusPendaftaranScreen> {
  ...
  Widget build(BuildContext ctx) => Scaffold(
    ...
    body: _loading
        ...
        : _registrations.isEmpty
            ? const Center(child: Text('Belum ada pendaftaran scrim.', style: TextStyle(color: BooyahTheme.textMuted)))
            : SingleChildScrollView(
      padding: const EdgeInsets.all(14),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const SectionHeader(title: 'PENDAFTARAN AKTIF'),
        ..._registrations.map((reg) => _buildStatusCard(
          title: reg['scrim_title'] as String? ?? 'Unknown',
          admin: '${reg['admin_name'] ?? 'Admin'} · ${_fmtDate(reg['scheduled_at'])} · ${_fmtTime(reg['scheduled_at'])}',
          regStatus: reg['reg_status'] as String? ?? 'unknown',
          roomId: reg['room_id'] as String?,
          borderColor: _getStatusColor(reg['reg_status'] as String? ?? 'unknown'),
          statusLabel: _getStatusLabel(reg['reg_status'] as String? ?? 'unknown'),
          statusColor: _getStatusColor(reg['reg_status'] as String? ?? 'unknown'),
        )),
      ]),
    ),
  );
  ...
  Color _getStatusColor(String status) {
    switch(status) {
      case 'waiting_verify': return BooyahTheme.yellow;
      case 'waiting_room_id': return BooyahTheme.yellow;
  ...
  String _getStatusLabel(String status) {
    switch(status) {
      case 'waiting_verify': return 'VERIFIKASI';
      case 'waiting_room_id': return 'MENUNGGU ROOM ID';
  ...
  static Widget _buildStatusCard({
    required String title, required String admin,
    required String regStatus, required String? roomId,
    required Color borderColor, required String statusLabel, required Color statusColor,
  }) =>
      Container(
        padding: const EdgeInsets.all(14),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(admin, style: const TextStyle(fontSize: 10, color: BooyahTheme.textMuted)),
            ])),
