...
class _VerifikasiKlaimScreenState extends State<VerifikasiKlaimScreen> {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Transfer ke ${claim['user_name']} dikonfirmasi! Notifikasi terkirim.'),
  ...
  Widget build(BuildContext ctx) => Scaffold(
        ...
        body: _loading
            ...
            : Column(
                children: [
                  // Summary bar lengkap dengan Menunggu, Selesai, dan Total 🚀
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                    color: BooyahTheme.surface,
                    child: Row(children: [
                      _summaryChip(Icons.hourglass_empty_rounded, 'Menunggu', '${_claims.length}', BooyahTheme.yellow),
                      const SizedBox(width: 14),
                      _summaryChip(Icons.check_circle_outline_rounded, 'Selesai', '$_successCount', BooyahTheme.green),
                      const SizedBox(width: 14),
                      _summaryChip(Icons.assignment_outlined, 'Total', '$_totalClaims', BooyahTheme.maroonB),
                    ]),
                  ),
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return Container(
                                ...
                                child: Column(children: [
                                  Row(children: [
                                    const Icon(Icons.sports_esports_rounded, size: 26, color: BooyahTheme.maroonGlow),
                                    const SizedBox(width: 10),
                                    Expanded(
                                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      Text(c['team_name'] as String? ?? 'Team', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                                      Text('${c['user_name']} · ${c['scrim_title']}', style: const TextStyle(fontSize: 10, color: BooyahTheme.textMuted)),
                                    ])),
                                    Text('Rp${_fmt(c['amount'] as int? ?? 0)}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: BooyahTheme.gold)),
                                  ]),
                                  const SizedBox(height: 8),
                                  Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8),
                                    decoration: BoxDecoration(color: BooyahTheme.surface, borderRadius: BorderRadius.circular(6)),
                                    child: Row(children: [
                                  ...
                                  Row(children: [
                                    Expanded(
                                      ...
                                      child: Container(
                                        ...
                                        child: const Text('✓ TRANSFER SELESAI',
                                            textAlign: TextAlign.center, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: BooyahTheme.green, letterSpacing: 0.5)),
                                      ),
                                    )),
  ...
  Widget _summaryChip(IconData icon, String label, String val, Color color) => Row(children: [
