...
class _RoomIdScreenState extends State<RoomIdScreen> {
  ...
  final _msgCtrl   = TextEditingController(text: 'Harap masuk room tepat waktu. GL HF semua!');
  int _selectedScrim = 0;
  bool _loading = false;

  final _scrims = [
    {'name': 'BOOYAH CUP SEASON 7', 'sched': '11 Mar · 19:00 WIB', 'count': '11/20 terverifikasi'},
    {'name': 'MIDNIGHT CLASH RANKED', 'sched': '11 Mar · 21:00 WIB', 'count': '16/16 terverifikasi'},
  ];
  ...
  void _send() async {
    ...
    if (_roomCtrl.text.isEmpty || _passCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('⚠️ Data tidak boleh kosong!'),
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('✅ Room ID terkirim ke semua peserta terverifikasi!'),
  ...
  Widget build(BuildContext ctx) => Scaffold(
    ...
    body: SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ..._scrims.asMap().entries.map((e) => GestureDetector(
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(e.value['name']!, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                Text('${e.value['sched']} · ${e.value['count']}',
                  style: const TextStyle(fontSize: 10, color: BooyahTheme.textMuted)),
              ])),
              if (_selectedScrim == e.key)
                const Icon(Icons.check_circle, color: BooyahTheme.maroonB, size: 20),
            ]),
          ),
        )),
        ...
        Container(
          ...
          child: const Row(children: [
            ...
            SizedBox(width: 8),
            Text('11 tim terverifikasi · siap menerima Room ID',
        ...
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('ROOM ID *', style: TextStyle(fontSize: 10, color: BooyahTheme.textMuted, letterSpacing: 0.8)),
            const SizedBox(height: 5),
            TextField(
              controller: _roomCtrl,
              keyboardType: TextInputType.number,
              inputFormatters: [FilteringTextInputFormatter.digitsOnly],
              style: const TextStyle(fontFamily: 'Orbitron', fontSize: 18, fontWeight: FontWeight.w700, color: BooyahTheme.textPri, letterSpacing: 3),
              decoration: const InputDecoration(hintText: '0000000'),
            ),
          ])),
          const SizedBox(width: 8),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('PASSWORD *', style: TextStyle(fontSize: 10, color: BooyahTheme.textMuted, letterSpacing: 0.8)),
            const SizedBox(height: 5),
            TextField(
        ...
        const Text('PESAN TAMBAHAN', style: TextStyle(fontSize: 10, color: BooyahTheme.textMuted, letterSpacing: 0.8)),
        const SizedBox(height: 5),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text('Room ID Tersedia',
              style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
            const SizedBox(height: 4),
            RichText(text: TextSpan(
              style: const TextStyle(fontFamily: 'Rajdhani', fontSize: 11, color: BooyahTheme.textSec),
              children: [
                const TextSpan(text: 'Room ID: '),
                TextSpan(text: _roomCtrl.text.isEmpty ? '??????' : _roomCtrl.text,
                  style: const TextStyle(color: BooyahTheme.gold, fontFamily: 'Orbitron', fontWeight: FontWeight.w700)),
                const TextSpan(text: ' · Password: '),
                TextSpan(text: _passCtrl.text.isEmpty ? '??????' : _passCtrl.text,
                  style: const TextStyle(color: BooyahTheme.gold, fontFamily: 'Orbitron', fontWeight: FontWeight.w700)),
              ],
            )),
          ]),
        ...
        BooyahButton(label: '🔑 KIRIM ROOM ID SEKARANG', onTap: _send, isLoading: _loading),
